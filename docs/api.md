...
- `Registry` has a usable, concurrency-safe zero value; `NewRegistry` is the
  explicit constructor. `Register` adds one unique method and `Lookup` supports
  inspection. Do not copy a registry after first use. An empty string is a
  valid method name because the protocol requires only a string; the `rpc.`
  prefix is reserved.
