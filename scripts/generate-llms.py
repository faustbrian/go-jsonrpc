...
RAW_BASE = "https://raw.githubusercontent.com/faustbrian/go-jsonrpc/main"
...
def documentation_paths(readme: str) -> list[pathlib.Path]:
    ...
    paths = []
    for target in re.findall(r"^- \[[^]]+\]\(([^)]+\.md)\)\s*$", match["body"], re.MULTILINE):
        path = ROOT / target
        if not path.is_file():
...
def render_full(readme: str, paths: list[pathlib.Path]) -> str:
    ...
    generated_notice = "<!-- Code generated by scripts/generate-llms.py; DO NOT EDIT. -->"
...
def main() -> int:
    ...
    if stale:
        print(
            "generated LLM documentation is stale: " + ", ".join(stale),
            file=sys.stderr,
        )
