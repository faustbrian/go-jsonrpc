...
if ! grep -Fq "[$version]: https://github.com/faustbrian/go-jsonrpc/releases/tag/$next" CHANGELOG.md; then
...
go test -run='^$' -fuzz=FuzzDispatcher -fuzztime=30s .
go test -run='^$' -fuzz=FuzzRequestUnmarshal -fuzztime=30s .
go test -run='^$' -fuzz=FuzzResponseUnmarshal -fuzztime=30s .
go test -run='^$' -fuzz=FuzzErrorUnmarshal -fuzztime=30s .
go test -run='^$' -fuzz=FuzzIDRoundTrip -fuzztime=30s .
go test -run='^$' -fuzz=FuzzClientCorrelation -fuzztime=30s .
go test -run='^$' -fuzz=FuzzClientBatchCorrelation -fuzztime=30s .
